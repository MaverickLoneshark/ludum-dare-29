	...
	CanvasManager.fn = CanvasManager.prototype = {
		...
		addCanvas : function() {
			...
			canvas[0].onmouseup = function(event)
			{
				...
				var y = event.pageY;
				ldGame.projectile_manager.initializeProjectile(Math.floor(Math.random() * 3), x, y/*, (Math.random() * 10) - 5, (Math.random() * 10) - 5*/);
			...
			for(var p = 0; p < projectiles.length; p++)
			{
					...
					(projectiles[p].y[ldGame.projectile_manager.TRAIL_LENGTH - 1] < offheight + CANVAS_HEIGHT + CANVAS_DRAW_BUFFER))))
				{
					...
					switch(projectiles[p].type)
					{
						...
						case 9:
							var date = new Date();
							var radius = (date.getTime() - projectiles[p].birth) / 23;
							...
							context.lineWidth = 1;
							context.arc(projectiles[p].x[0], projectiles[p].y[0] - offheight, radius, 0, 2 * Math.PI);
							context.fill();
						break;
