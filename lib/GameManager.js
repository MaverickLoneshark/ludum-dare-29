...
(function() {
	...
	function generateEnemyProjectile(currentTime)
	{
		...
		if(this.nextMissile <= currentTime)
		{
			...
			var target = ldGame.game_manager.getRandomBuilding();
			if (!target) return;
			var target_x = (target.x + (target.width /2)) * GRID_PIXEL_SIZE; //(Math.random() * ldGame.canvas_manager.CANVAS_WIDTH);
			var target_y = (target.y + (target.height/2)) * GRID_PIXEL_SIZE;
			
			var velocity = calculateVelocity(origin_x, 0, target_x, target_y);//ldGame.canvas_manager.CANVAS_HEIGHT * ldGame.canvas_manager.canvases.length);
	...
	GameManager.fn = GameManager.prototype = {
		...
		getRandomBuilding : function(bclass) {
			...
			var rand = Math.floor(Math.random() * applicable.length);
		...
		removeBuilding : function(bld) {
			for (var x = bld.x; x < bld.x + bld.width; x++) {
				for (var y = bld.y; y < bld.y + bld.height; y++) {
		...
		buildingCollision : function(x, y) {
			...
			var bld = this.grid[y][x]; //convert to grid coords
			if (!bld) return false;
